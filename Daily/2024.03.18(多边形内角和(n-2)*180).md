...
void solve()
{
    ...
    if( !turn_left( a[0] , a[1] , a[2] ) ) reverse( a.begin() , a.end() ) ;

    LD sum = 0 ;
    for( int i = 0 , j = 1 , k = 2 ; i < n ; i++ , j++ , k++ )
    {
        if( !turn_left( a[i] , a[j%n] , a[k%n] ) )
        {
            cout << "No\n" ; return ;
        }
        point v1 = a[i]-a[j%n] , v2 = a[k%n]-a[j%n] ;
        LD arg = ((LD)atan2( v1.y , v1.x ) - (LD)atan2( v2.y , v2.x )) ;
        if( sgn( arg ) < 0 ) arg += PI*2 ;
        sum += arg ;
    }
    if( sgn( sum-PI*(n-2) ) == 0 ) cout << "Yes\n" ;
    else cout << "No\n" ;
