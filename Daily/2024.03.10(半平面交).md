https://codeforces.com/gym/104736/problem/H

```cpp
//pbds
/*
history_bug1 : 输入直线时没有注意起点终点使得保持原点在内侧的设置，导致每条线的半平面交求的侧面不同
*/
#include<bits/stdc++.h>
using namespace std ;
#define ll long long
const int inf = 1e8 ;
const int N = 3e6+9 ;
typedef long double LD ;
const LD eps = 1e-7 ;
const LD box = 1e7 ;
int sgn( LD x )
{
    return x > eps ? 1 : ( x < -eps ? -1 : 0 ) ;
}
struct point
{
    LD x , y ;
    point operator + (const point &a) const
    {
        return{ x+a.x , y+a.y } ;
    }
    point operator - (const point &a) const
    {
        return{ x-a.x , y-a.y } ;
    }
    point operator * (const LD &a) const
    {
        return{ x*a , y*a } ;
    }
    point operator / (const LD &a) const
    {
        return{ x/a , y/a } ;
    }

    friend bool operator == ( const point &a , const point &b )
    {
        return sgn(a.x-b.x) == 0 && sgn(a.y-b.y) == 0 ;
    }

    friend ostream & operator << ( ostream &out , point &a )
    {
        out << "(" << a.x << "," << a.y << ")" ;
        return out ;
    }
    int idx , hull_rank ;
};
struct line
{
    point s , t ;
    friend bool operator == ( const line &a , const line &b )
    {
        return (a.s==b.s&&a.t==b.t)||(a.s==b.t&&a.t==b.s) ;
    }
};
LD sqr( LD x )
{
    return x*x ;
}
LD mo( point x )
{
    return sqrtl( sqr(x.x) + sqr(x.y) ) ;
}
LD dis( const point &a , const point &b )
{
    return sqrtl( sqr(a.x-b.x) + sqr(a.y-b.y) ) ;
}
LD dot( const point &a , const point &b )
{
    return a.x*b.x+a.y*b.y;
}
LD det( const point &a , const point &b )
{
    // + : b 在 a 的逆时针方向
    return a.x*b.y-b.x*a.y ;
}
bool point_on_segment( const point &a , const line &l )
{
    if( l.s == l.t )
    {
        return a == l.s ;
    }
    return sgn( det(l.s-a,a-l.t) ) == 0 && sgn( dot(l.s-a,l.t-a) ) <= 0 ;
}
bool two_side( const point &a , const point &b , const line &c )
{
    // 点是否在线两边
    return sgn( det(a-c.s,c.t-c.s) ) * sgn( det(b-c.s,c.t-c.s) ) < 0 ;
}
bool segment_inter_judge( const line &a , const line &b )
{
    bool ok = 0 ;
    ok |= point_on_segment( b.s , a ) ;
    ok |= point_on_segment( b.t , a ) ;
    ok |= point_on_segment( a.s , b ) ;
    ok |= point_on_segment( a.t , b ) ;
    ok |= ( two_side(a.s,a.t,b)&&two_side(b.s,b.t,a) ) ;
    return ok ;
}
bool ray_inter_judge( const line &a , const line &b )
{
    //前提都不是退化的射线
    return sgn( det( a.t-a.s , b.t-b.s ) ) == 0 ? 0 : 1 ;
}
LD point_to_line( const point &p , const line &l )
{
    return fabs( det(l.t-l.s,p-l.s) )/dis(l.s,l.t) ;
}
LD point_to_segment( const point &p , const line &l )
{
    if( l.s == l.t ) return dis( p , l.s ) ;
    if( sgn( dot(l.s-p,l.t-l.s) )*sgn( dot(l.t-p,l.t-l.s) ) <= 0 ) return point_to_line(p,l) ;
    else return min( dis(p,l.s) , dis(p,l.t) ) ;
}
LD arg( const line &a , const line &b )
{
    LD res = dot( a.t-a.s , b.t-b.s ) ;
    res /= mo( a.t-a.s ) ;
    res /= mo( b.t-b.s ) ;
    return acos( res ) ;
}
point line_intersect( const line &a , const line &b )
{
    LD u = det( a.t-a.s,b.s-a.s ) ;
    LD v = -det( a.t-a.s , b.t-a.s ) ;
    return ( b.s*v + b.t*u )/(v+u) ;
}
bool turn_left( const point &a , const point &b , const point &c )
{
    // a 是中间点，看看ac是不是在ab的左边（逆时针方向）
    return sgn( det( b-a , c-a ) ) > 0 ;
}
bool turn_left( const line &a , const line &b , const line &c )
{
    // bc的交点是不是在向量a的左侧
    return turn_left( a.s , a.t , line_intersect(b,c) ) ;
}
int point_quadrant( const point &a )
{
    if( sgn( a.x ) >= 0 && sgn( a.y ) >= 0 ) return 1 ;
    else if( sgn(a.x) < 0 && sgn( a.y ) >= 0 ) return 2 ;
    else if( sgn(a.x) < 0 && sgn( a.y ) < 0 ) return 3 ;
    else return 4 ;

}
vector<point> convex_hull( vector<point> a )
{
    vector<point>ret ;
    int a_size = a.size() , ret_size = 0 ;
    if( a_size <= 2 ) return a ;
    sort( a.begin() , a.end() , []( point x , point y ){ return x.x == y.x ? x.y < y.y : x.x < y.x ; } )  ;
    for( int i = 0 ; i < a_size ; i ++ )
    {
        while( ret_size > 1 && !turn_left( ret[ret_size-2] , ret[ret_size-1] , a[i] ) )
        {
            ret.pop_back() ; ret_size -- ;
        }
        ret.push_back( a[i] ) ; ret_size ++ ;
    }
    int fix = ret_size ;
    for( int i = a_size-2 ; i >= 0 ; i -- )
    {
        while( ret_size > fix && !turn_left( ret[ret_size-2] , ret[ret_size-1] , a[i] ) )
        {
            ret.pop_back() ; ret_size -- ;
        }
        ret.push_back( a[i] ) ; ret_size ++ ;
    }
    ret.pop_back() ; ret_size -- ;

    return ret ;
}
int half( const point &a ) //在上下哪个平面
{
    return a.y > 0 || ( a.y == 0 && a.x > 0 ? 1 : 0 );
}
bool is_para( const line &a , const line &b ) //是不是共线
{
    return sgn( det(a.t-a.s,b.t-b.s) ) == 0 ;
}
bool cmp( const line &a , const line &b )
{
    // (0,0) is org
    int sign = half( a.t-a.s ) - half( b.t-b.s ) ; //是否在同一半平面
    int dir = sgn( det(a.t-a.s,b.t-b.s) ) ; //是否共线

    if( sign == 0 && dir == 0 ) return sgn( det(a.t-a.s , b.t-a.s) ) < 0 ; //距离原点近的
    else return sign ? sign > 0 : dir > 0 ;
    //如果异面，上半平面优先
    //如果同面，逆时针排序
}
vector<line> A ;
LD D ;
vector<point> hpi( int siz_a )
{
    vector<line>h ;
    for( int i = 0 ; i < siz_a ; i ++ ) h.push_back( A[i] ) ;
    h.push_back( {{box,box} , {-box,box}} ) ;
    h.push_back( {{-box,box} , {-box,-box}} ) ;
    h.push_back( {{-box,-box} , {box,-box}} ) ;
    h.push_back( {{box,-box} , {box,box}} ) ;
    sort( h.begin() , h.end() , cmp ) ;
/*
    for( auto u : h )
    {
        cout << "{" << u.s << "," << u.t << "}\n" ;
    }
*/
    vector<line> q( h.size() ) ;
    int l = 0 , r = -1 ;
    for( auto &i : h )
    {
        while( l<r && !turn_left(i,q[r-1],q[r]) ) --r ;
        while( l<r && !turn_left(i,q[l],q[l+1]) ) ++l ;
        if( l <= r && is_para(i,q[r]) ) continue ;
        q[++r] = i ;
    }
    while( r-l>1 && !turn_left(q[l],q[r-1],q[r]) ) --r ;
    while( r-l>1 && !turn_left(q[r],q[l],q[l+1]) ) ++l ;

    if( r-l < 2 ) return {} ;
    vector<point> ret(r-l+1) ;
    for( int i = l ; i <= r ; i ++ )
        ret[i-l] = line_intersect( q[i] , q[i==r?l:i+1] ) ;
    return ret ;
}
bool on_box( point a )
{
    if( sgn(a.x-box) == 0 || sgn(a.y-box) == 0 ) return 1 ;
    return 0;
}
bool is_open( vector<point>ret )
{
    if( ret.size() <= 2 )
    for( auto &u : ret ) if( on_box(u) ) return 1 ;
    return 0 ;
}
bool check( int x )
{
    vector<point> ret = hpi( x ) ;
    if( is_open(ret) ) return 0 ;

    int m = ret.size() ;
    for( int i = 0 ; i < m ; i ++ )
    {
        if( sgn(dis({0,0},ret[i])-D) >= 0 ) return 0 ;
    }
    return 1 ;
}
void solve()
{
    int n ; cin >> n >> D ;
    A.resize(n) ;

    for( int i = 0 ; i < n ; i ++ )
    {
        cin >> A[i].s.x >> A[i].s.y >> A[i].t.x >> A[i].t.y ;
        if( !turn_left( A[i].s , A[i].t , {0,0} ) ) swap( A[i].s , A[i].t ) ;
    }

    int l = 1 , r = n , ans = 0 , mid ;
    while( l <= r )
    {
        mid = l+r >> 1 ;
        if( check( mid ) ) ans = mid , r = mid-1 ;
        else l = mid+1 ;
    }

    if( !ans ) cout << "*\n" ;
    else cout << ans << "\n" ;

}
int main()
{
    ios::sync_with_stdio(false);cin.tie(0);cout.tie(0);
    int tt = 1 ; //cin >> tt ;
    while( tt-- ) solve() ;
    return 0 ;
}
/*
5 4.321
-2 -1 3 -2
1 6 3 -2
1 6 -2 -1
-3 4 3 3
-2 1 5 4
*/
```

[https://vjudge.net/article/4491#:~:text=7-,QOJ%2D2162,-Domes
](https://vjudge.net/problem/QOJ-2162)

```cpp
/*
history_bug1: 全部的关系都要考虑到，不然面积不准确
history_bug2: 关同步流以后不要用 puts
半平面交的是向量左边的半平面，极角排序选哪个点都可以
*/
#include<bits/stdc++.h>
using namespace std ;
#define ll long long
const int inf = 1e8 ;
const int N = 3e6+9 ;
typedef long double LD ;
const LD eps = 1e-7 ;
const LD box = 1e7 ;
int sgn( LD x )
{
    return x > eps ? 1 : ( x < -eps ? -1 : 0 ) ;
}
struct point
{
    LD x , y ;
    point operator + (const point &a) const
    {
        return{ x+a.x , y+a.y } ;
    }
    point operator - (const point &a) const
    {
        return{ x-a.x , y-a.y } ;
    }
    point operator * (const LD &a) const
    {
        return{ x*a , y*a } ;
    }
    point operator / (const LD &a) const
    {
        return{ x/a , y/a } ;
    }
    friend bool operator == ( const point &a , const point &b )
    {
        return sgn(a.x-b.x) == 0 && sgn(a.y-b.y) == 0 ;
    }

    friend ostream & operator << ( ostream &out , point &a )
    {
        out << "(" << a.x << "," << a.y << ")" ;
        return out ;
    }
    int idx , hull_rank ;
};
struct line
{
    point s , t ;
    friend bool operator == ( const line &a , const line &b )
    {
        return (a.s==b.s&&a.t==b.t)||(a.s==b.t&&a.t==b.s) ;
    }
};
LD sqr( LD x )
{
    return x*x ;
}
LD mo( point x )
{
    return sqrtl( sqr(x.x) + sqr(x.y) ) ;
}
LD dis( const point &a , const point &b )
{
    return sqrtl( sqr(a.x-b.x) + sqr(a.y-b.y) ) ;
}
LD dot( const point &a , const point &b )
{
    return a.x*b.x+a.y*b.y;
}
LD det( const point &a , const point &b )
{
    // + : b 在 a 的逆时针方向
    return a.x*b.y-b.x*a.y ;
}
bool point_on_segment( const point &a , const line &l )
{
    if( l.s == l.t )
    {
        return a == l.s ;
    }
    return sgn( det(l.s-a,a-l.t) ) == 0 && sgn( dot(l.s-a,l.t-a) ) <= 0 ;
}
bool two_side( const point &a , const point &b , const line &c )
{
    // 点是否在线两边
    return sgn( det(a-c.s,c.t-c.s) ) * sgn( det(b-c.s,c.t-c.s) ) < 0 ;
}
bool segment_inter_judge( const line &a , const line &b )
{
    bool ok = 0 ;
    ok |= point_on_segment( b.s , a ) ;
    ok |= point_on_segment( b.t , a ) ;
    ok |= point_on_segment( a.s , b ) ;
    ok |= point_on_segment( a.t , b ) ;
    ok |= ( two_side(a.s,a.t,b)&&two_side(b.s,b.t,a) ) ;
    return ok ;
}
bool ray_inter_judge( const line &a , const line &b )
{
    //前提都不是退化的射线
    return sgn( det( a.t-a.s , b.t-b.s ) ) == 0 ? 0 : 1 ;
}
LD point_to_line( const point &p , const line &l )
{
    return fabs( det(l.t-l.s,p-l.s) )/dis(l.s,l.t) ;
}
LD point_to_segment( const point &p , const line &l )
{
    if( l.s == l.t ) return dis( p , l.s ) ;
    if( sgn( dot(l.s-p,l.t-l.s) )*sgn( dot(l.t-p,l.t-l.s) ) <= 0 ) return point_to_line(p,l) ;
    else return min( dis(p,l.s) , dis(p,l.t) ) ;
}
LD arg( const line &a , const line &b )
{
    LD res = dot( a.t-a.s , b.t-b.s ) ;
    res /= mo( a.t-a.s ) ;
    res /= mo( b.t-b.s ) ;
    return acos( res ) ;
}
point line_intersect( const line &a , const line &b )
{
    LD u = det( a.t-a.s,b.s-a.s ) ;
    LD v = det( a.t-a.s , b.t-a.s ) ;
    return ( b.s*v - b.t*u )/(v-u) ;
}
bool turn_left( const point &a , const point &b , const point &c )
{
    // a 是中间点，看看ac是不是在ab的左边（逆时针方向）
    return sgn( det( b-a , c-a ) ) > 0 ;
}
bool turn_left( const line &a , const line &b , const line &c )
{
    // bc的交点是不是在向量a的左侧
    return turn_left( a.s , a.t , line_intersect(b,c) ) ;
}
int point_quadrant( const point &a )
{
    if( sgn( a.x ) >= 0 && sgn( a.y ) >= 0 ) return 1 ;
    else if( sgn(a.x) < 0 && sgn( a.y ) >= 0 ) return 2 ;
    else if( sgn(a.x) < 0 && sgn( a.y ) < 0 ) return 3 ;
    else return 4 ;

}
vector<point> convex_hull( vector<point> a )
{
    vector<point>ret ;
    int a_size = a.size() , ret_size = 0 ;
    if( a_size <= 2 ) return a ;
    sort( a.begin() , a.end() , []( point x , point y ){ return x.x == y.x ? x.y < y.y : x.x < y.x ; } )  ;
    for( int i = 0 ; i < a_size ; i ++ )
    {
        while( ret_size > 1 && !turn_left( ret[ret_size-2] , ret[ret_size-1] , a[i] ) )
        {
            ret.pop_back() ; ret_size -- ;
        }
        ret.push_back( a[i] ) ; ret_size ++ ;
    }
    int fix = ret_size ;
    for( int i = a_size-2 ; i >= 0 ; i -- )
    {
        while( ret_size > fix && !turn_left( ret[ret_size-2] , ret[ret_size-1] , a[i] ) )
        {
            ret.pop_back() ; ret_size -- ;
        }
        ret.push_back( a[i] ) ; ret_size ++ ;
    }
    ret.pop_back() ; ret_size -- ;

    return ret ;
}
int half( const point &a ) //在上下哪个平面
{
    return a.y > 0 || ( a.y == 0 && a.x > 0 ? 1 : 0 );
}
bool is_para( const line &a , const line &b ) //是不是共线
{
    return sgn( det(a.t-a.s,b.t-b.s) ) == 0 ;
}
bool cmp( const line &a , const line &b )
{
    // (0,0) is org
    int sign = half( a.t-a.s ) - half( b.t-b.s ) ; //是否在同一半平面
    int dir = sgn( det(a.t-a.s,b.t-b.s) ) ; //是否共线

    if( sign == 0 && dir == 0 ) return sgn( det(a.t-a.s , b.t-a.s) ) < 0 ; //距离原点近的
    else return sign ? sign > 0 : dir > 0 ;
    //如果异面，上半平面优先
    //如果同面，逆时针排序
}
vector<line> A ;
LD DX , DY ;
vector<point> hpi( int siz_a )
{
    vector<line>h ;
    for( int i = 0 ; i < siz_a ; i ++ ) h.push_back( A[i] ) ;

    h.push_back( {{DX,DY} , {0,DY}} ) ;
    h.push_back( {{0,DY} , {0,0}} ) ;
    h.push_back( {{0,0} , {DX,0}} ) ;
    h.push_back( {{DX,0} , {DX,DY}} ) ;
    sort( h.begin() , h.end() , cmp ) ;
/*
    for( auto u : h )
    {
        cout << "{" << u.s << "," << u.t << "}\n" ;
    }
*/
    vector<line> q( h.size()+10 ) ;
    int l = 0 , r = -1 ;
    for( auto &i : h )
    {
        while( l<r && !turn_left(i,q[r-1],q[r]) ) --r ;
        while( l<r && !turn_left(i,q[l],q[l+1]) ) ++l ;
        if( l <= r && is_para(i,q[r]) ) continue ;
        q[++r] = i ;
    }
    while( r-l>1 && !turn_left(q[l],q[r-1],q[r]) ) --r ;
    while( r-l>1 && !turn_left(q[r],q[l],q[l+1]) ) ++l ;

    if( r-l < 2 ) return {} ;
    vector<point> ret(r-l+1) ;
    for( int i = l ; i <= r ; i ++ )
        ret[i-l] = line_intersect( q[i] , q[i==r?l:i+1] ) ;
    return ret ;
}
bool on_box( point a )
{
    if( sgn(a.x-box) == 0 || sgn(a.y-box) == 0 ) return 1 ;
    return 0;
}
bool is_open( vector<point>ret )
{
    if( ret.size() <= 2 )
    for( auto &u : ret ) if( on_box(u) ) return 1 ;
    return 0 ;
}
void solve()
{
    int n ; cin >> DX >> DY >> n ;
    vector<point>B(n) ;

    for( int i = 0 ; i < n ; i ++ )
    {
        cin >> B[i].x >> B[i].y ;
    }

    vector<int>v(n) ;
    for( int i = 0 ; i < n ; i ++ ) cin >> v[i] ;

    if( n == 1 )
    {
        cout<< fixed << setprecision(6) << DX*DY<<"\n";
        return ;
    }

    for( int i = 0 ; i < n ; i ++ )
        for( int j = i+1 ; j < n ; j ++ )
            A.push_back( { B[ v[j]-1 ] , B[v[i]-1] } ) ;


    vector<point>p = hpi( A.size() ) ;

    //for( auto u : p ) cout << u << "\n" ;

    LD ans = 0 ;
    int m = p.size() ;
    if( m == 0 )
    {
        cout << "0\n" ;
        return ;
    }
    else if( m == 1 )
    {
        cout<< fixed << setprecision(6) << DX*DY<<"\n";
        return ;
    }
    p.push_back( p[0] ) ;
    for( int i = 0 ; i < m ; i ++ )
    {
        ans += det( p[i] , p[i+1] );
    }


    ans /= 2.0 ;

    cout << fixed << setprecision(6) << ans << "\n" ;

}
int main()
{
    ios::sync_with_stdio(false);cin.tie(0);cout.tie(0);
    int tt = 1 ; //cin >> tt ;
    while( tt-- ) solve() ;
    return 0 ;
}
/*
5 4.321
-2 -1 3 -2
1 6 3 -2
1 6 -2 -1
-3 4 3 3
-2 1 5 4
*/
```
